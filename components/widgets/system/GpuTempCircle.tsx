...
export function GpuTempWidget({ widget, isEditMode, onRemove }: WidgetProps) {
    ...
    // Color based on load
    const color = gpu > 80 ? '#ef4444' : gpu > 50 ? '#a855f7' : '#8b5cf6'; // Purple for GPU
                ...
                <div className="flex-1 w-full relative flex items-center justify-center">
                    ...
                    <div className="relative w-24 h-24">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center flex-col">
                            <span className="text-xl font-bold text-white">{gpu}%</span>
                            {memoryUsed > 0 && (
                                <span className="text-[10px] text-white/50">{Math.round(memoryUsed / 1024)}GB</span>
                            )}
                        </div>
                    </div>
                </div>
